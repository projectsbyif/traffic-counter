...
models.Records.count().then((count) => {
  if (count === 0) {
    const NOW = moment().utc().minute(0).second(0).millisecond(0);
    const TWELVE_HOURS_AGO = moment().utc().subtract(12, 'hours').minute(0)
    ...
    createNewRecord(moment().utc()).then(() => {
      return process.exit();
    });
  }
});
...
let createNewRecord = (hour) => {
  return new Promise((resolve, reject) => {
    models.Records.create({
      ...
      timestamp: hour.format('YYYY-MM-DD HH:mm'),
    })
    .then(() => resolve());
  });
};
